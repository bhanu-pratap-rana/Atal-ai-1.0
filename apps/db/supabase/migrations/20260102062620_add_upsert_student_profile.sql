...
COMMENT ON FUNCTION upsert_student_profile IS 'Atomic UPSERT for student profiles - prevents race condition in concurrent saves. Uses ON CONFLICT clause to ensure only one record per user_id exists.';;
