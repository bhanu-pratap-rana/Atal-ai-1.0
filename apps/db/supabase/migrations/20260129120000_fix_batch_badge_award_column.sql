-- =====================================================
-- Migration: Fix batch_check_and_award_badges column name
-- =====================================================
--
-- BUG FIX: Previous migrations use wrong column name 'reason' instead of 'source'
--
-- PROBLEM:
-- The points_history table has column 'source' (defined in migration 042)
-- But batch_check_and_award_badges uses 'reason' which causes INSERT to fail
--
-- IMPACT: Badge points fail to insert - students don't get points for badges!
--
-- SOLUTION: Recreate the function with correct column name 'source'
-- =====================================================
  ...
  LOOP
    ...
    CASE v_badge.unlock_criteria->>'type'
      ...
      WHEN 'high_score' THEN
        -- Check for high score on summative assessments
        SELECT COUNT(*) INTO v_count
    ...
    IF v_earned THEN
      ...
      -- FIX: Changed 'reason' to 'source' (correct column name per migration 042)
...
'FIXED: Uses source column (not reason) in points_history.';
